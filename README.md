...
Inside this repository, you will find several problem set folders for Week 1:
* `world/` - A basic "hello, world" exercise to get you started.
* `me/` - A basic exercise prompting for your name.
* `mario-less/` - Build a right-aligned half-pyramid of blocks. (Easier version)
* `mario-more/` - Build two adjacent half-pyramids of blocks. (More comfortable version)
* `cash/` - Implement a greedy algorithm to calculate minimum coins for change. (Easier version)
* `credit/` - Implement Luhn's Algorithm to validate credit card numbers. (More comfortable version)
...
1. `git remote add upstream https://github.com/emowat/codingrabbit-cs50-week1.git` *(one-time setup — links you to the original course repository; skip this if you've already done it once)*
